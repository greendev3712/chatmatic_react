...
class ActionSubmit extends React.Component {
  state = {
    workflowUid: this.props.campaignAdd.workflowUid,
  ...
  UNSAFE_componentWillReceiveProps(nextProps) {
    this.setState({
      workflowUid: nextProps.campaignAdd.workflowUid,
  ...
  render() {
    const renderMessageAlert = () => {
      if (!this.state.workflowUid) {
        ...
        const selectedWorkflow = this.props.workflows.find(
          workflow => workflow.uid === this.state.workflowUid
        );

        if (!selectedWorkflow) {
          toastr.error('The workflow in the campaign does not exist any more');
          return (
            <Redirect
              to={{
                pathname: `/page/${this.props.match.params.id}/campaigns`
              }}
            />
          );
        }
        ...
        return (
          <div className="d-flex flex-column align-items-center">
            <span className="d-flex align-items-center align-items-center success-alert">
              ...
              {`Using Engagement: ${selectedWorkflow.name}`}
            </span>
            <button
