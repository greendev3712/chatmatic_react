...
export default class DragBoard extends Component {
    ...
    componentDidMount = () => {
        this.handleAutoSave();
        this.loadImages();
        this.handleBoardScale(this.state.scale);
        this.setBoradHeightWidthScale();
    };

    UNSAFE_componentWillReceiveProps = nextProps => {
        const { newItem, scale, autoArrange, data } = nextProps;
        const {
    ...
    setBoradHeightWidthScale = () => {
        ...
        if (heightElement) {
            ...
            const trnsXMove = 0 - ((width / 2) * scale) + Math.round(minLeft * scale) + 50 + Math.round((offsetWidth - (width * scale)) / 2);
            const trnsYMove = 0 - (Math.round((height / 2) - ((height / 2) * scale)) + Math.round(minTop * scale)) + 50 + Math.round((offsetHeight - (height * scale)) / 2);
            document.getElementById('drag-borad').style.transformOrigin = `${Math.abs(trnsXMove) + 50}px center`;
    ...
    handleBoardScale = (scale, height = null, width = null) => {
        ...
        board.style.transform = `scale(${scale}) translate(${trnsXMove}px, ${trnsYMove}px)`;
        board.style.transformOrigin = `${Math.abs(trnsXMove) + 50}px ${Math.abs(trnsYMove) + 50}px`;
        setTimeout(() => {
    ...
    handleTranslate = e => {
        ...
        const originX = e.clientX + e.width / 2;
        this.setState({
            trnsX,
            trnsY
        });
    ...
    handleTranslateDrag = e => {
        ...
        board.style.transformOrigin = `${Math.abs(trnsXMove) + 50}px center`;
    ...
    handleScale = () => {
        const scaled = window.scrollY / (2000 - window.innerHeight);
        const box = document.getElementById('drag-borad');
    ...
    _drawSVG = (
        ...
        if (isBtn) {
            ...
            if (btn) {
                y4 =
                    parentTop < childTop
                        ? btn.offsetTop + (btn.offsetHeight / 2 - 7)
                        : svgNewHeight -
                        parentHeight +
                        (btn.offsetTop + (btn.offsetHeight / 2 - 7));
            }
